...
- environment：docs/scripts-only / local shallow repository / no DB/provider/media
- evidence_level：E1 + independent executable gate review
- 命令：`npm run test:governance`；`npm run governance:git-budget -- --worktree`；低阈值无 ADR/有 ADR 对照；`--staged`；无参 degraded fallback；`git diff --check`
- fixture：20/21 文件、2000/2001 行、ADR 放行
- contract：Git budget、ADR 和 workflow 规则与协作文档一致
- unit：15/15 通过；worktree 为 16 files / 1189 net additions；低阈值无 ADR 正确失败，当前 diff 内 ADR 正确放行
- API：N/A
- 浏览器 trace：N/A
- DB/MySQL/Prisma：N/A
- API 请求/响应安全摘要：N/A
...
- 复核范围：NUL-safe、rename/binary/untracked/staged、merge-base/shallow、ADR in-diff/count/expiry/commit、预算内 ADR 忽略、workflow 空 ADR 参数。
