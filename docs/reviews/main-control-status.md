...
| 维度 | 当前状态 | 主控判断 |
| --- | --- | --- |
| 小说核心流程 | mock/in-memory 主流程可演示，已知个案已定向修复 | 真实 Prisma 后半程未实现、全书审稿未输入正文，存在 2 个未关闭 P0；没有可重复的全链浏览器 E2E |
...
| 整改计划 | RP-00A 已正式关闭 | 唯一总账已关闭 3/42；当前按依赖进入 RP-00B，全部关闭前不进入新需求 |
...
- 暂缓 `P10-R1`，不得按编号自动继续。
- `RP-00A` 已完成 TEST/PRODUCT/QUALITY 独立复核、主控裁决和远程推送；下一步只推进 `RP-00B`。
...
- 全栈研发会话：`RP-00A` docs-only 交付已完成，当前 idle。
- 测试会话：`RP-00A` 三个 acceptance id 正式验收全部 approved，当前 idle。
...
- 最新工程质量：二次复盘归并 42 项 PB/RB/QG/DEBT 和专项验证缺口；`RP-00A` 已关闭 3 项 QG，当前关闭 3/42。
...
- 一次性 `apps/api/tsconfig.testrun.json` 已进入 RP-00B 归因处理：当前核对为无脚本引用、输出 `/tmp/apidist2`、排除单项测试，按一次性辅助文件删除，待独立验收；`.playwright-cli/` 继续作为本地浏览器运行产物忽略。
...
1. `RP-00A 状态与证据基线` 已正式关闭；按依赖只派发 `RP-00B Git 规模门禁、派发 SLA、临时文件归因`。
2. `RP-00B` 完成后再按依赖图推进；管理分组不得整体派发。每个子包独立研发、测试、关闭、commit 和 push。
3. 小说真实完本金丝雀通过后，再执行 P9-real；P10-R1 只在 `RP-10` 重新决策。
4. 继续保持真实 DB/provider、外部媒体和平台发布的独立授权门禁。

不得提前执行 `RP-01A` 及后续子包、`P9-real`、`P10-R1`、CS-L1、真实 MySQL/provider 或外部发布能力。
