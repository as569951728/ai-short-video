...
| 测试 | R1 验收准备完成，当前空闲 | `acceptance_prepared=yes`，无缺失信息；研发交付后可立即正式验收 |
...
- `P10-R1` 已按 preflight、domain-memory、Prisma static、regression 四组完成验收准备；授权研发并完成交付后，再执行自动化与 in-memory API live 正式验收。
