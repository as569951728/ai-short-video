...
- 内部持久化：`idempotencyToken / requestHash`，只用于幂等判定，不进入前端可见 `VideoPublishRecordDTO`
- `createdBy / createdAt / updatedAt`

### 5.2 VideoPublishFreezeSnapshot

冻结以下版本 ID、`versionNo` 和安全摘要：
...
- `overdueAt = dueAt + graceWindow`
- `overdueAt` 由后续策略配置在创建节点时计算并冻结；R0 不写死宽限小时数
- 持久状态：`pending | completed | waived`
- 派生状态：`waiting | due | overdue`
- `completedAt / waivedReason`

创建发布记录时同步创建两个节点，不使用定时器。时间派生只作用于 `pending` 节点：

- `now < dueAt`：`waiting`
- `dueAt <= now < overdueAt`：`due`
- `now >= overdueAt`：`overdue`
- `completed / waived` 为终态，不再派生 `due / overdue`

`dueAt`、`overdueAt` 必须存在，且 `overdueAt >= dueAt`；后续若允许零宽限窗口，必须由策略显式给出并补边界测试。查询列表或详情时，根据冻结时间与当前时间惰性计算，不依赖后台任务。
...
所有写接口必须携带 `idempotencyToken`，但响应 DTO 不回显该 token：
...
必须审计：登记发布、数据回填、更正、撤销、样本不足判断、下一步决策、删除或归档保护。审计保存操作人、租户、原因、前后值安全摘要和冻结版本 refs。前后值摘要必须使用明确的字段白名单 DTO，禁止任意 `Record<string, unknown>` 或透传原始对象。
...
R0 请求合同必须为后续乐观锁保留明确版本字段：创建登记携带 `videoUnitId / expectedReferenceVersion / expectedExportVersionNo / expectedRenderVersionNo`；发布更正和撤销携带 `expectedVersionNo`；指标回填携带 `expectedBackfillNodeVersionNo`。客户端不得通过 `expectedExportStatus` 声明服务端真实状态，R1 必须从仓储读取并校验当前 export/render 状态。
...
2. `P10-R0`：只冻结 shared DTO、枚举、错误码、安全摘要、脱敏规则、无定时器纯时间函数与测试；仅定义审计字段口径，不实现 route/service/repository/Prisma/UI。
