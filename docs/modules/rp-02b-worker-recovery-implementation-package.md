...
状态：`rp02b1_completed_rp02b2a_single_superseded_split_round6_approved_asset_submission_pending`
...
RP-02B 不作为单个大包直接实现，完整顺序共十个阶段。`RP-02B1` 已完成；其后九个阶段仍须逐包推进：

1. `RP-02B1`：可恢复执行合同、lease/fencing 字段和仓储原语。
2. `RP-02B2a0`：high/blocking 试写确认原因的同步全链前置修复。
3. `RP-02B2a1`：registry、严格 provider ABI 与公开 retry 冻结。
4. `RP-02B2a2`：可信 actor、canonical envelope、权威重载和 provider 前 gate。
5. `RP-02B2a3`：expected-phase CAS、attempt 时机、fail-safe 与历史 retry child fencing；正常 leased provider=0。
6. `RP-02B2a4`：InMemory 15-action action-specific fenced finalize。
7. `RP-02B2a5`：Prisma 9/6 capability 与 fenced finalize，收口 B2a E3。
8. `RP-02B2b`：queued claim、HTTP 快速返回、heartbeat 和 worker 生命周期。
9. `RP-02B2c`：前端最小 transport 适配。
10. `RP-02B3`：restart recovery、真实 retry、poison/unknown outcome、预算和 deterministic 故障注入。

每个阶段独立满足：需求复核、实现、自测、独立 TEST/QUALITY、commit/push、远程 clean-checkout CI。B2a0/B2a1-B2a5/B2b/B2c 的当前权威合同见 `docs/modules/rp-02b2-dispatcher-transport-implementation-package.md`；本文件旧 B2 合并描述不再作为可派发写集。B1-B3 均完成 E3 后，`RMD-TASK-002/003` 最多进入 `implemented_pending_verification`；真实 MySQL、多进程和进程 kill/restart 的 E6 仍依赖 `RP-01D`，不得提前关闭。
...
### RP-02B2a0 / RP-02B2a1-B2a5 / RP-02B2b / RP-02B2c

- B2a0 目标：high/blocking 试写确认原因从 Admin 到同步 provider 前校验的真实全链。
- B2a1-B2a5 依次完成：registry/strict ABI/公开 retry freeze；authority reload/pre-stale；phase/attempt/fail-safe；InMemory finalize；Prisma 9/6 finalize。
- B2b 目标：queued claim、HTTP 202、worker lifecycle/heartbeat、graceful shutdown。
- B2c 目标：admin 真实 taskId、稳定幂等键、精确轮询和刷新/多标签恢复。
- 各阶段唯一有效写集、路径 manifest、预算、详细合同和独立验收清单均以 `docs/modules/rp-02b2-dispatcher-transport-implementation-package.md` 为准；本文和历史评审中的旧数值全部失效。
- 前一阶段完成前不得授权下一阶段；这些阶段都不实现完整 RP-02C 任务投影与取消产品语义。
...
- RP-02B2a0/B2a1-B2a5/B2b/B2c 的独立 fixture、命令与场景已由 `docs/modules/rp-02b2-dispatcher-transport-implementation-package.md` 第 11-12 节完整替代。
- `CrashableWorker`、recovery、restart、retry child、unknown outcome、poison、renewal-vs-recovery 只属于 RP-02B3，不得计入 B2 子包准入。
- B1 的 envelope/legacy 回归继续由 `test:rp02b1` 独立执行，不得被 B2 测试目录污染。
- RP-02B3 的最终 deterministic 清单必须在 B2c 完成后重新复核；旧 25 项混合清单不再作为任何子包的验收依据。
...
RP-02B 后续子包只有在以下条件满足后才能派发：

1. 后端架构、产品交互、TEST、QUALITY 对专属实现包复核为 approved，P0/P1 为 0。
2. 验收矩阵和整改计划同步分阶段口径。
3. 需求资产 commit/push 且治理 CI 成功。
4. MC 按 B2a0、B2a1、B2a2、B2a3、B2a4、B2a5、B2b、B2c、B3 分别授权；实现会话不得自动进入下一包。
