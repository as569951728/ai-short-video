...
状态：`rp02b1_completed_rp02b2_replanned`
...
RP-02B 不作为单个 15 文件大包直接实现，拆为六个顺序子包：

1. `RP-02B1`：可恢复执行合同、lease/fencing 字段和仓储原语。
2. `RP-02B2a0`：high/blocking 试写确认原因的同步全链前置修复。
3. `RP-02B2a`：worker dispatcher、权威重载、checkpoint、stale 双门禁和原子 fenced finalize。
4. `RP-02B2b`：queued claim、HTTP 快速返回、heartbeat 和 worker 生命周期。
5. `RP-02B2c`：前端最小 transport 适配。
6. `RP-02B3`：restart recovery、真实 retry、poison/unknown outcome、预算和 deterministic 故障注入。

每个子包独立满足：需求复核、实现、自测、独立 TEST/QUALITY、commit/push、远程 clean-checkout CI。B2a0/B2a/B2b/B2c 的当前权威合同见 `docs/modules/rp-02b2-dispatcher-transport-implementation-package.md`；本文件旧 B2 合并描述不再作为可派发写集。B1-B3 均完成 E3 后，`RMD-TASK-002/003` 最多进入 `implemented_pending_verification`；真实 MySQL、多进程和进程 kill/restart的 E6 仍依赖 `RP-01D`，不得提前关闭。
...
### RP-02B2a0 / RP-02B2a / RP-02B2b / RP-02B2c

- B2a0 目标：high/blocking 试写确认原因从 Admin 到同步 provider 前校验的真实全链。
- B2a 目标：dispatcher、15-action authority matrix、checkpoint、双 stale gate、原子 fenced finalize。
- B2b 目标：queued claim、HTTP 202、worker lifecycle/heartbeat、graceful shutdown。
- B2c 目标：admin 真实 taskId、稳定幂等键、精确轮询和刷新/多标签恢复。
- 四包唯一有效写集、路径 manifest、预算、详细合同和独立验收清单均以 `docs/modules/rp-02b2-dispatcher-transport-implementation-package.md` 为准；本文和历史评审中的旧数值全部失效。
- 前一包完成前不得授权下一包；四包都不实现完整 RP-02C 任务投影与取消产品语义。
...
- RP-02B2a0/B2a/B2b/B2c 的独立 fixture、命令与场景已由 `docs/modules/rp-02b2-dispatcher-transport-implementation-package.md` 第 11-12 节完整替代。
...
4. MC 按 B2a0、B2a、B2b、B2c、B3 分别授权；实现会话不得自动进入下一包。
