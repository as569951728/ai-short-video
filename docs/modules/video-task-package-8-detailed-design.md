...
- `/videos` 已先落成 P8 步骤式承接工作台，用前端 mock/view-model 支撑产品体验验收。
- 已实现：视频承接概览、5 步承接步骤条、待视频化小说来源、视频项目列表、创建视频项目 4 步向导、引用快照查看、引用重新检测、引用异常处理和后续生成灰态说明。
- 创建向导当前在前端内存中创建 `VideoProject`、`VideoReference` 和默认 `VideoUnit` 的视图模型，用于验证交互；后端持久化接口、真实 DTO 和数据库写入后续按本设计继续接入。
- 步骤入口使用 `/videos?focus=source|list|snapshot|issue|generation` 和 `/videos?create=1&novelId=...`，刷新后能回到对应步骤。
- P8 页面不提供可执行的旁白、TTS、字幕、渲染、导出、发布或数据回填动作；相关内容只作为灰态后续阶段说明。

### 当前验收收口状态

- 2026-06-23：P8 前端步骤式承接工作台通过测试会话复验，当前阶段可以收口。
- 收口复盘见 `docs/reviews/video-p8-acceptance-closure-2026-06-23.md`。
- 本次通过范围是前端产品层和 mock/view-model 闭环，不代表后端持久化、真实 MySQL 写路径、接口幂等和长任务化引用检测已经完成。
- 后续建议先拆 P8b：补齐视频承接后端持久化和接口接入，再进入 P9 视频生成工作台研发。P8b 详细设计见 `docs/modules/video-task-package-8b-detailed-design.md`。
- P9/P10 不随 P8 自动启动，必须等待用户明确确认。
