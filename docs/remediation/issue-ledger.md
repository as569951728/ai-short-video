...
| RMD-NOV-VERSION-001 | RB/P1 | current 唯一性缺数据库强约束，真实产物来源可能仍记为 `mock_ai` | partial | `prismaNovelRepository.ts`；N-14；NOV-CANDIDATE-04 的真实 MySQL current 唯一性 E6 未证明 | DEV + QUALITY | RP-03A, RP-03D | NOV-CURRENT-01, NOV-CANDIDATE-04, NOV-PROVENANCE-01 |
| RMD-NOV-UX-001 | RB/P1 | 候选的编辑、优化、融合、结果定位、采用后导航和历史版本交互不统一；章节目录与试写主 CTA 也曾出现交互不一致或点击无效果 | closed | PR #61 merge `91751dd`；PRODUCT/TEST/QUALITY 第七代码候选 `P0=0/P1=0`；shared 23/23、API 126/126、admin 78/78、DOM 21/21、typecheck 与远端 required checks 全绿；真实浏览器原事故复验；关闭记录 | DEV + PRODUCT | RP-05A, RP-05B, RP-05D | NOV-CANDIDATE-01, NOV-CANDIDATE-02, NOV-CANDIDATE-03, NOV-CANDIDATE-UI-01, NOV-CANDIDATE-05, NOV-CANDIDATE-06, NOV-AI-CTA-01 |
...
| RMD-TASK-002 | PB/P0 | 请求内 await 长 provider，没有独立 worker、心跳和进程重启恢复 | partial | RP-02B1 `415d03a` 与 RP-02B2a0 `2da6d31` 已通过限定阶段验收；RP-02B2a1 accepted head `4817abc` 已完成 15-action registry、strict provider ABI 和公开 retry freeze。RP-02B2a2 admitted candidate `e8e37cd`、delivery `dc193db`、post-merge gate `9f04986` 已完成 authenticated actor、15-action authority reload/stale gate、active-claim fence 与 legacy fail closed，trusted replay `29977969717`、ARCH/SECURITY `0/0/1`、TEST/QUALITY `0/0/0` 均通过；阶段证据见 `docs/reviews/remediation-rmd-task-002-003-rp-02b2a2-verification-2026-07-23.md`。B2a3-B2a5/B2b/B2c/B3、独立 worker/heartbeat/restart、真实 DB/provider/media/E6 未实现或未证明 | DEV | RP-02B | TASK-WORKER-01, TASK-RESTART-01 |
| RMD-TASK-003 | RB/P1 | 通用重试只创建 queued 记录，没有消费者实际重新执行 | open | RP-02B1 `415d03a` 只建立 root budget/attempt/recovery 合同；RP-02B2a1 accepted head `4817abc` 只冻结 provider-backed 公开重试为 `409 RETRY_NOT_AVAILABLE`、`retryable=false` 和零副作用。RP-02B2a2 `e8e37cd` 继续回归 retry freeze，并保证 legacy/无效 envelope 不进入 provider、lease 或 recovery，但没有创建或消费真实 retry child；阶段证据见 `docs/reviews/remediation-rmd-task-002-003-rp-02b2a2-verification-2026-07-23.md`。真实 retry child 创建、预算/lineage、消费者执行与 restart/unknown outcome 仍未实现 | DEV + TEST | RP-02B | TASK-RETRY-01 |
...
| RMD-GOV-TYPECHECK-001 | QG/P1 | 隔离工作树缺少本地 `node_modules` 时，Node/TypeScript 向上解析到父工作树的旧 workspace 链接，造成 API shared 合同漂移的假阻塞 | closed | 关闭记录 `docs/reviews/remediation-rmd-gov-typecheck-001-closure-2026-08-02.md`；隔离工作树执行 `npm ci` 后 workspace 链接均落在本工作树，根级 shared/admin/api typecheck 全绿；未修改业务合同 | DEV + QUALITY | RP-00A | GOV-TYPECHECK-01 |
