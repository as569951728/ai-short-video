...
| 验收 ID | 核心操作与断言 | 最低证据 | 阻塞条件 |
| --- | --- | --- | --- |
| TASK-PRECLAIM-01 | 分阶段验收：RP-02A 证明 provider 前原子创建/复用 task；B2a0 证明风险参数在同步 provider 前完整校验，已由 `2da6d31`、API 50/Admin 17/DOM 6、TEST/QUALITY 与远程 clean checkout 固定；RP-02B1/B2a 补安全 ExecutionEnvelope 与可恢复执行；B2b 以同一 trusted actor 完成 capability→携带 expected scope/revision/expiry 的 POST→task/events poll，服务端 claim 前重读权威 policy snapshot、constant-time compare scope、精确匹配 revision/expiry 并以 authoritative clock 拒绝过期，首请求返回 `202 + queued`；B2c 前端保存真实 taskId 且不得误报完成 | RP-02A/B1/B2a0/B2a/B2b/B2c E3 + 最终 E6 | 默认 context 创建 B2 envelope；capability 与提交/轮询身份不一致；expected scope/revision/expiry 缺失、不匹配或过期仍 claim；provider 先调用；风险确认在 provider 后才校验；fire-and-forget；202 后显示已生成或清 pending；本地假 taskId |
| TASK-CONCURRENCY-01 | RP-02B2a 验证 checkpoint/finalize fencing、authority 行锁/CAS TOCTOU 阻断与双租户隔离；B2b 用 10-worker barrier 连续 20 轮验证唯一 lease/provider/finalize，用 barrier 证明 settling/abandoned 各自胜出，并在客户端复核后/服务端 claim 前改变 capability revision 或推进 authoritative clock 越过 expiry 时证明零 claim；B2c 以 `expectedSubmissionId+expectedRevision` CAS、分区单调 revision 和 CAS 后 capability 二次门禁阻断旧标签/TOCTOU 重复 POST；同 scope attestation 轮换时只能受限 CAS 刷新 capability revision/expiry 并保持 submission/key/request/hash 不变，二次复核后才可恢复；恢复场景必须分别固定 queued→`202 accepted reused=true`、waiting/terminal→`200 existing reused=true`，不得使用 202/200 宽松并集断言；B3 再补 recovery/retry 竞争；真实 MySQL/多进程待 E6 | RP-02A/B1/B2a/B2b/B2c/B3 E3 + 最终 E6 | matcher 后 authority 可变；shutdown 与 settlement 双写；旧 submission/revision 覆盖新记录；客户复核后 revision/过期变化仍 claim；刷新 attestation 时改写 submission 身份或请求；只实现 queued 或 terminal 其中一条恢复路径却通过；顺序请求冒充 worker 竞争；重复 provider/asset/receipt；跨租户复用；in-memory 外推真实 DB |
| TASK-WORKER-01 | B2a：15-action authority matrix、provider public ABI 只接 action-specific strict `Pick<>`、provider 前/事务 finalize 内 stale gate、lease-CAS checkpoint、成功与受控失败的 task+asset/receipt+operation-log+event 同事务，既存 retry child provider 前终态阻断，Prisma 9 支持/6 不支持能力门禁；B2a 交付必须由 fail-fast core+B2a0、既有 route retry 回归、远程 workflow 和 clean checkout 串行 B1+B2a 共同证明；B2b：PausedScheduler 下 POST 在 provider=0 返回 202，tick 后唯一 processing和 heartbeat/shutdown；B2c：admin transport 正确 | B2a/B2b/B2c 独立 E3；真实 HTTP latency、DB 事务和独立 worker 进程留 E6 | raw repository entity/cast 进入 provider；B2a 新 fixture 未进入远程 CI；跳过 B1/B2a0 回归；HTTP await provider；旧 finalize 后补 receipt；过期 owner可写；retry child 调用 provider、写业务资产或创建新 child；unsupported 先排队；queued 显示调用模型 |
| TASK-RESTART-01 | B2a/B2b 只证明 checkpoint、heartbeat、防接管、过期 owner scan 前零写和 graceful shutdown 让 lease 自然过期；grace timeout 后本地 execution=abandoned，迟到 resolve/reject/abort 即使 lease 尚有效也 repository 零写；recovery CAS、restart、unknown outcome 归 B3；真实 kill/restart 归 E6 | B2a/B2b E3 + B3 E3 + E6 | B2 误称已恢复；无 owner/token/expiry；abandoned 回调写 fail/finalize；shutdown 立即 requeue；盲目重放 provider |
| TASK-RETRY-01 | B2a/B2b/B2c 全程 provider-backed retry 稳定 `409 RETRY_NOT_AVAILABLE`、task projection `retryable=false`、固定“未写入新内容/当前阶段暂不支持任务重试”失败原因与 disabled 下一步、retry mutation/child=0；B3 才实现 tenant+parent+token 原子 child、预算/stale/active claim/lineage、worker 实际消费 | B2 execution-core + route DTO/409/文案/零副作用回归 + B3 E3 + E6 | B2 UI 暴露可用 retry；隐藏按钮后仍用“稍后重试”误导；retry 409 创建 event/log/child；worker 消费不完整 queued child；child 重置预算；多个 active child；unknown 自动重试 |
...
| 验收 ID | 核心操作与断言 | 最低证据 | 阻塞条件 |
| --- | --- | --- | --- |
| GOV-GIT-01 | 每包独立 commit/push；超过 20 文件或 2,000 行需同 diff ADR；ADR 必须在实现 diff 中改为 `status=ready`，写实际 file/addition、不可分拆原因、owner 与有效期/commit；远程 CI 必须按 PR/push 的同一 BASE/HEAD、NUL-safe 发现 changed ADR 并显式传入 budget gate；upstream 对齐 | E1 + Git hook/CI check | 预先存在但本包未修改的 ADR；status 非 ready；无参/HEAD~1 fallback；ADR 未传入；实际计数不符；跨两个已验收包继续累积 |
