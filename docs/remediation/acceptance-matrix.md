...
| GOV-TYPECHECK-01 | 在 latest main clean worktree 执行根级 `npm run typecheck`，shared、admin-web、api 全部退出 0；修复必须保持 execution-envelope 版本、actor helper 和 ErrorCode 的单一 shared 合同 | E1 + 根级 typecheck + 独立 QUALITY | 任一 workspace 类型失败；靠跳过 API、使用旧 dist 或删除检查通过 |
