...
| TASK-WORKER-01 | B2a：15-action authority matrix、provider public ABI 只接 action-specific strict `Pick<>`、provider 前/事务 finalize 内 stale gate、lease-CAS checkpoint、成功与受控失败的 task+asset/receipt+operation-log+event 同事务，既存 retry child provider 前终态阻断，Prisma 9 支持/6 不支持能力门禁；B2b：PausedScheduler 下 POST 在 provider=0 返回 202，tick 后唯一 processing和 heartbeat/shutdown；B2c：admin transport 正确 | B2a/B2b/B2c 独立 E3；真实 HTTP latency、DB 事务和独立 worker 进程留 E6 | raw repository entity/cast 进入 provider；HTTP await provider；旧 finalize 后补 receipt；过期 owner可写；retry child 调用 provider、写业务资产或创建新 child；unsupported 先排队；queued 显示调用模型 |
...
| TASK-RETRY-01 | B2a/B2b/B2c 全程 provider-backed retry 稳定 `409 RETRY_NOT_AVAILABLE`、task projection `retryable=false`、retry mutation/child=0；B3 才实现 tenant+parent+token 原子 child、预算/stale/active claim/lineage、worker 实际消费 | B2 冻结回归 + B3 E3 + E6 | B2 UI 暴露可用 retry；worker 消费不完整 queued child；child 重置预算；多个 active child；unknown 自动重试 |
