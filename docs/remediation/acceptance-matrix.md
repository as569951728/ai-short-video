...
| 验收 ID | 核心操作与断言 | 最低证据 | 阻塞条件 |
| --- | --- | --- | --- |
| GOV-PROGRESS-01 | 从唯一总账计算 PB/RB/QG/DEBT 原始关闭计数；机器快照、主控状态和执行方案必须一致。连续两个合并 PR 没有总账状态前进时进入 `execution_reset`、活动实现包为空；新包必须声明目标问题、预期状态迁移、用户结果和证据桶 | E1 + `npm run governance:progress` + 独立 QUALITY 复核 | 使用完成百分比；PR/测试/Agent 数替代用户结果；快照与总账不一致；达到停工阈值仍有活动实现包；按编号自动启动后续包 |
| GOV-TYPECHECK-01 | 在 latest main clean worktree 先执行 workspace-local `npm ci`，确认 `node_modules/@ai-shortvideo/shared` 等 workspace 链接解析到当前工作树而非父目录，再执行根级 `npm run typecheck`；shared、admin-web、api 必须全部退出 0，并保持 execution-envelope 版本、actor helper 和 ErrorCode 的单一 shared 合同 | E1 + workspace-local install/link evidence + 根级 typecheck + 独立 QUALITY | 任一 workspace 类型失败；依赖解析逃逸到父工作树；靠跳过 API、使用旧 dist 或删除检查通过 |
...
| GOV-STATUS-01 | main-control 只保留当前状态；历史进入事件账本；旧巡检不得覆盖。2026-08-02 回归关闭以 PR #59、merge `e3cdc9a` 和同头四路 main push success 为远端证据 | E1 + consistency check + remote required checks | 同一文件互相冲突；普通治理 push 被历史 B2a admission receipt 劫持；仅本地通过但远端主干失败 |
