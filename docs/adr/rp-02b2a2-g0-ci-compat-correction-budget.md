...
The final amended package remains exactly nine manifest files with 372 additions, 64 deletions, and 308 net additions from the fixed baseline. G0-C1 rejects both files outside the manifest and any candidate missing one of the nine frozen files. The `10 / 700` hard ceiling leaves review contingency but does not authorize an unnamed tenth file; any manifest or budget change requires an ADR revision before admission. The final non-concurrent fixed regression snapshot passed 50/50 in 1,461.23 seconds after the old-sibling workflow binding correction; this local timing is not used as remote timeout evidence.
