import{describe,it}from"node:test";import assert from"node:assert/strict";import{execFileSync,spawnSync}from"node:child_process";import{createHash}from"node:crypto";import{chmodSync,mkdtempSync,mkdirSync,readFileSync,realpathSync,symlinkSync,unlinkSync,writeFileSync}from"node:fs";import{tmpdir}from"node:os";import{dirname,resolve}from"node:path";import{PACKAGE_DEFINITIONS,analyzePackageGate}from"./rp02b2a-package-gate.mjs";const ROOT=resolve("."),GATE_SCRIPT="scripts/rp02b2a-package-gate.mjs",WORKFLOW=resolve(".github/workflows/rp01c-fixtures.yml"),ADMISSION_WORKFLOW=resolve(".github/workflows/rp02b2a-admission.yml"),TRUSTED_WORKFLOW_ORACLE_SHA256="36c418d2ffc07b4701225e654ca84b77a4488d913adc418cdf63cc8c4ccd77a4",
BASELINE="501a3cfcdf12341d9f611f0fdd6a6336d4ade483",B2A2_BASELINE="6eaf60af4155a8b95ff77d53261f5896d3a8f77d",STALE_B2A2_BASELINE="4817abc67cf916772b317aff027403b97ab4df76",A1_ADR="docs/adr/rp-02b2a1-registry-abi-budget.md",GATE_PREP_ID="RP-02B2a2-G0",GATE_PREP_ADR="docs/adr/rp-02b2a2-gate-prep-budget.md",VERIFIED_GATE_PREP="verified-gate-prep";const GOVERNANCE_REMEDIATION_FILES=Object.freeze([".github/workflows/rp01a-e2e.yml",".github/workflows/rp01b-dom.yml",".github/workflows/rp01c-fixtures.yml",".github/workflows/remediation-governance.yml",".github/workflows/rp02b2a-admission.yml","apps/api/test/rp01c/fixtureFactory.test.ts","docs/modules/rp-02b2-dispatcher-transport-implementation-package\
...
test:rp02b2a1:gate","RP-02B2a2":"test:rp02b2a2","RP-02B2a3":"test:rp02b2a3","RP-02B2a4":"test:rp02b2a4","RP-02B2a5":"test:rp02b2a5"});const RANGE_PACKAGES=Object.freeze(["RP-02B2a3","RP-02B2a4","RP-02B2a5"]);const ORACLE=Object.freeze({"RP-02B2a1":["RP-02B2a1-v1",A1_ADR,18,1900],[GATE_PREP_ID]:["RP-02B2a2-G0-v1",GATE_PREP_ADR,15,2e3],"RP-02B2a2":["RP-02B2a2-v1","docs/adr/rp-02b2a2-authority-claim-budget.md",18,1900],"RP-02B2a3":["RP-02B2a3-v1","docs/adr/rp-02b2a3-lease-retry-budget.md",14,1800],"\
...
docs/adr/"))){if(file===GATE_SCRIPT||packageId===GATE_PREP_ID&&GOVERNANCE_REMEDIATION_FILES.includes(file))write(path,file,readFileSync(resolve(ROOT,file)));else write(path,file,`${file}
...
function copyCurrentFiles(repoPath,files){for(const file of files)write(repoPath,file,readFileSync(resolve(ROOT,file)))}function addMinimalB2a2Candidate(repoPath,base){const[,adrPath,hardMaxFiles,hardMaxNetAdditions]=ORACLE["RP-02B2a2"];for(const file of select("RP-02B2a2").filter(item=>item!==adrPath))write(repoPath,file,`${file}
...
it("keeps candidate-owned gate, test, and oracle out of the trusted G0 checkout", () => {
  ...
  for (const [options, pattern] of [
    ...
    [{ eventRef: "refs/heads/release", workflowRef: "example/repo/.github/workflows/rp02b2a-admission.yml@refs/heads/release" }, /repository default branch/],
...
function finalEvidenceText(file, existing, parent, net, fields, gateCount = `${GATE_TEST_COUNT}/${GATE_TEST_COUNT}`) {
  ...
  if (file === GATE_PREP_EVIDENCE_FILES[1]) return existing
    ...
    .replace(/^\| RP-02B2a2-G0 整改后最终复核 \|.*$/m, `| RP-02B2a2-G0 整改后最终复核 | 已完成 | accepted code head \`${short}\`；15 files / ${net} net additions；package gate ${gateCount}；四路远程 CI completed/success；B2a2 保持 not_authorized |`)
