...
const POSTMERGE_RANGE_CORRECTION_ID = 'RP-02B2a2-PM-C1', POSTMERGE_RANGE_CORRECTION_BASELINE_SHA = 'dc193dbbd3ac1970f571fd618f12902a4033994c', POSTMERGE_RANGE_CORRECTION_ADR = 'docs/adr/rp-02b2a2-postmerge-range-correction-budget.md';
const CLOSEOUT_POLICY_CORRECTION_ID = 'RP-02B2a2-PM-C2', CLOSEOUT_POLICY_CORRECTION_BASELINE_SHA = '9f04986469a3e409b3ce887390e8830cbdfe9493', CLOSEOUT_POLICY_CORRECTION_ADR = 'docs/adr/rp-02b2a2-closeout-policy-correction-budget.md';
const CI_TRIGGER_SPLIT_ID = 'RP-02B2a2-PM-C3', CI_TRIGGER_SPLIT_BASELINE_SHA = '0ff9107bbfbf95983ae8c21391754520f4374711', CI_TRIGGER_SPLIT_ADR = 'docs/adr/rp-02b2a2-ci-trigger-split-budget.md';
const A2_CLOSEOUT_EVIDENCE_ID = 'RP-02B2a2-E3', A2_CLOSEOUT_EVIDENCE_COMMAND = 'test:governance', A2_CLOSEOUT_EVIDENCE_MAX_ADDITIONS = 180, A2_CLOSEOUT_EVIDENCE_MAX_DELETIONS = 80, A2_CLOSEOUT_EVIDENCE_MAX_NET_ADDITIONS = 180;
const A2_CLOSEOUT_EVIDENCE_VERIFICATION = 'docs/reviews/remediation-rmd-task-002-003-rp-02b2a2-verification-2026-07-23.md';
const A2_CLOSEOUT_EVIDENCE_FILES = new Set(['docs/remediation/acceptance-matrix.md', 'docs/remediation/issue-ledger.md', 'docs/reviews/main-control-event-ledger.md', 'docs/reviews/main-control-status.md', A2_CLOSEOUT_EVIDENCE_VERIFICATION]);
...
const TRUSTED_ADMISSION_WORKFLOW_LEGACY_HEADS = new Set([
  ...
  G0_CORRECTION_BASELINE_SHA,
  G0_TEST_LAYER_BASELINE_SHA,
  G0_EVIDENCE_TRIGGER_BASELINE_SHA,
  G0_A2_SCOPE_BASELINE_SHA,
  G0_A2_SCOPE_V5_BASELINE_SHA,
  ACCEPTED_G0_C1_SHA,
  ACCEPTED_G0_C3_SHA,
  ACCEPTED_G0_C4_SHA,
  '056a8d28910c765c9887a245e2dc4269859e5ec2',
  'e8e37cd892570e3abee961374ec3512a373f6400',
  POSTMERGE_RANGE_CORRECTION_BASELINE_SHA,
  CLOSEOUT_POLICY_CORRECTION_BASELINE_SHA,
]);
...
const FIXED_BASELINES = Object.freeze({ 'RP-02B2a1': B2A1_BASELINE_SHA, [B2A2_GATE_PREP_ID]: B2A2_GATE_PREP_BASELINE_SHA, [G0_CORRECTION_ID]: G0_CORRECTION_BASELINE_SHA, [G0_TEST_LAYER_ID]: G0_TEST_LAYER_BASELINE_SHA, [G0_EVIDENCE_TRIGGER_ID]: G0_EVIDENCE_TRIGGER_BASELINE_SHA, [G0_A2_SCOPE_ID]: G0_A2_SCOPE_BASELINE_SHA, [G0_A2_SCOPE_V5_ID]: G0_A2_SCOPE_V5_BASELINE_SHA, [POSTMERGE_RANGE_CORRECTION_ID]: POSTMERGE_RANGE_CORRECTION_BASELINE_SHA, [CLOSEOUT_POLICY_CORRECTION_ID]: CLOSEOUT_POLICY_CORRECTION_BASELINE_SHA, [CI_TRIGGER_SPLIT_ID]: CI_TRIGGER_SPLIT_BASELINE_SHA, 'RP-02B2a2': GATE_PREP_BASELINE });
...
export const PACKAGE_DEFINITIONS = {
  ...
  [POSTMERGE_RANGE_CORRECTION_ID]: definition(POSTMERGE_RANGE_CORRECTION_ID, 'RP-02B2a2-PM-C1-v1', POSTMERGE_RANGE_CORRECTION_ADR, 'test:rp02b2a1:gate', 5, 1900, `.github/workflows/rp01c-fixtures.yml|apps/api/test/rp01c/fixtureFactory.test.ts|scripts/rp02b2a-package-gate.mjs|scripts/rp02b2a-package-gate.test.mjs|${POSTMERGE_RANGE_CORRECTION_ADR}`.split('|'), ['governance', 'test', 'adr']),
  [CLOSEOUT_POLICY_CORRECTION_ID]: definition(CLOSEOUT_POLICY_CORRECTION_ID, 'RP-02B2a2-PM-C2-v1', CLOSEOUT_POLICY_CORRECTION_ADR, 'test:rp02b2a1:gate', 5, 1500, `.github/workflows/rp01c-fixtures.yml|.github/workflows/rp02b2a-admission.yml|scripts/rp02b2a-package-gate.mjs|scripts/rp02b2a-package-gate.test.mjs|${CLOSEOUT_POLICY_CORRECTION_ADR}`.split('|'), ['governance', 'test', 'adr']),
  [CI_TRIGGER_SPLIT_ID]: definition(CI_TRIGGER_SPLIT_ID, 'RP-02B2a2-PM-C3-v1', CI_TRIGGER_SPLIT_ADR, 'test:rp02b2a1:gate', 5, 1200, `.github/workflows/rp01c-fixtures.yml|apps/api/test/rp01c/fixtureFactory.test.ts|scripts/rp02b2a-package-gate.mjs|scripts/rp02b2a-package-gate.test.mjs|${CI_TRIGGER_SPLIT_ADR}`.split('|'), ['governance', 'test', 'adr']),
...
const A2_CLOSEOUT_EVIDENCE_DEFINITION = Object.freeze({ packageId: A2_CLOSEOUT_EVIDENCE_ID, manifestId: 'RP-02B2a2-E3-v1', adrPath: 'none', testCommand: A2_CLOSEOUT_EVIDENCE_COMMAND, hardMaxFiles: 5, hardMaxNetAdditions: A2_CLOSEOUT_EVIDENCE_MAX_NET_ADDITIONS, baselinePolicy: 'repository-policy-source', manifest: A2_CLOSEOUT_EVIDENCE_FILES, requiredCategories: ['governance-evidence'] });
...
export function analyzePackageGate({ files, netAdditions, addedLines = netAdditions, deletedLines = 0, adrTextByPath, base, head, worktree = false, allowFallbackOverlap = false }) {
  ...
  const changedGovernanceCorrection = changedAdrs.includes(CI_TRIGGER_SPLIT_ADR) ? CI_TRIGGER_SPLIT_ID : changedAdrs.includes(CLOSEOUT_POLICY_CORRECTION_ADR) ? CLOSEOUT_POLICY_CORRECTION_ID : changedAdrs.includes(POSTMERGE_RANGE_CORRECTION_ADR) ? POSTMERGE_RANGE_CORRECTION_ID : changedAdrs.includes(G0_A2_SCOPE_V5_ADR) ? G0_A2_SCOPE_V5_ID : changedAdrs.includes(G0_A2_SCOPE_ADR) ? G0_A2_SCOPE_ID : changedAdrs.includes(G0_EVIDENCE_TRIGGER_ADR) ? G0_EVIDENCE_TRIGGER_ID : changedAdrs.includes(G0_TEST_LAYER_ADR) ? G0_TEST_LAYER_ID : changedAdrs.includes(G0_CORRECTION_ADR) ? G0_CORRECTION_ID : undefined;
  ...
  if (isCorrection) {
    const unexpected = changedAdrs.filter((path) => !G0_CORRECTION_ADRS.has(path));
    if (unexpected.length > 0) throw new Error(`${G0_CORRECTION_ID} changed ADR set violation: ${unexpected.join(', ')}`);
  } else if (changedAdrs.length !== 1) throw new Error(`RP-02B2a package gate requires exactly one changed ADR, got ${changedAdrs.length}`);
  const primaryAdr = isCorrection ? G0_CORRECTION_ADR : changedAdrs[0];
  const definition = Object.values(PACKAGE_DEFINITIONS).find((item) => item.adrPath === primaryAdr);
  ...
  if ([G0_CORRECTION_ID, G0_TEST_LAYER_ID, G0_EVIDENCE_TRIGGER_ID, G0_A2_SCOPE_ID, G0_A2_SCOPE_V5_ID, POSTMERGE_RANGE_CORRECTION_ID, CLOSEOUT_POLICY_CORRECTION_ID, CI_TRIGGER_SPLIT_ID].includes(definition.packageId) && !sameFileSet(changedFiles, definition.manifest)) {
...
function acceptedCandidateReceipt(input) {
  const candidate = input.authorizedCandidateSha, tree = input.authorizedCandidateTree, digest = input.authorizedManifestDigest;
  if (!candidate && !tree && !digest) return undefined;
  if (!candidate || !tree || !digest) throw new Error('RP-02B2a package gate rejects incomplete admitted candidate SHA/tree/manifest receipt');
  assertUsableSha('AUTHORIZED_CANDIDATE', candidate); assertGitCommit('AUTHORIZED_CANDIDATE', candidate);
  if (!/^[0-9a-f]{40,64}$/i.test(tree)) throw new Error(`RP-02B2a package gate rejects unparseable AUTHORIZED_CANDIDATE_TREE: ${tree}`);
  if (!/^[0-9a-f]{64}$/i.test(digest)) throw new Error(`RP-02B2a package gate rejects unparseable AUTHORIZED_MANIFEST_DIGEST: ${digest}`);
  const range = externallyAuthorizedRange(input, candidate), definition = PACKAGE_DEFINITIONS[input.authorizedPackageId];
  const actualTree = gitText(['rev-parse', `${candidate}^{tree}`]).trim();
  if (actualTree !== tree) throw new Error(`RP-02B2a admitted candidate tree mismatch: ${actualTree} != ${tree}`);
  const authorization = validateBusinessAuthorization({
    packageId: input.authorizedPackageId,
    base: range.base,
    head: range.head,
    gateSource: input.gateSource,
    g0EvidenceSha: input.g0EvidenceSha,
    authorizedPackageId: input.authorizedPackageId,
    authorizedPredecessorSha: input.authorizedPredecessorSha,
  });
  const actualDigest = authorization?.manifestDigest ?? packageManifestDigest(definition, range);
  if (actualDigest !== digest) throw new Error(`RP-02B2a admitted candidate manifest digest mismatch: ${actualDigest} != ${digest}`);
  return { ...range, candidate, tree, digest };
}
function samePathDescriptor(leftRef, rightRef, path) {
  return JSON.stringify(commitPathDescriptor(leftRef, path)) === JSON.stringify(commitPathDescriptor(rightRef, path));
}
function trustedSquashDeliveryRange(input, deliveryBase, deliveryHead, { requireMainRef } = {}) {
  const receipt = acceptedCandidateReceipt(input);
  if (!receipt) return undefined;
  if (requireMainRef !== undefined && requireMainRef !== 'refs/heads/main') throw new Error('RP-02B2a admitted squash delivery is accepted only on refs/heads/main');
  if (input.authorizedPackageId === 'RP-02B2a2') {
    if (deliveryBase !== input.g0EvidenceSha) throw new Error('RP-02B2a2 admitted squash delivery base must equal the accepted G0-E1 evidence SHA');
    validateAcceptedGatePrepEvidence({ gateSource: input.gateSource, evidenceHead: input.g0EvidenceSha });
  }
  assertUsableSha('DELIVERY_BASE', deliveryBase); assertUsableSha('DELIVERY_HEAD', deliveryHead);
  assertGitCommit('DELIVERY_BASE', deliveryBase); assertGitCommit('DELIVERY_HEAD', deliveryHead);
  const commitAndParents = gitText(['rev-list', '--parents', '-n', '1', deliveryHead]).trim().split(/\s+/);
  if (commitAndParents.length !== 2 || commitAndParents[1] !== deliveryBase) throw new Error('RP-02B2a admitted squash delivery must be one direct child of the event delivery base');
  const candidateFiles = [...new Set(changedFiles(receipt.base, receipt.head))].sort();
  const deliveredFiles = [...new Set(changedFiles(deliveryBase, deliveryHead))].sort();
  if (candidateFiles.length === 0 || JSON.stringify(candidateFiles) !== JSON.stringify(deliveredFiles)) throw new Error('RP-02B2a admitted squash delivery changed-path set does not match the admitted candidate');
  for (const path of candidateFiles) {
    if (!samePathDescriptor(receipt.base, deliveryBase, path)) throw new Error(`RP-02B2a admitted squash sibling base modified candidate-owned path: ${path}`);
    if (!samePathDescriptor(receipt.head, deliveryHead, path)) throw new Error(`RP-02B2a admitted squash delivery content differs from candidate: ${path}`);
  }
  return { base: receipt.base, head: receipt.head, deliveryBase, deliveryHead, rangeMode: 'trusted_squash_post_merge' };
}
function correctionBootstrapRange(head) {
  const commitAndParents = gitText(['rev-list', '--parents', '-n', '1', head]).trim().split(/\s+/);
  for (const packageId of [CI_TRIGGER_SPLIT_ID, CLOSEOUT_POLICY_CORRECTION_ID, POSTMERGE_RANGE_CORRECTION_ID, G0_A2_SCOPE_V5_ID, G0_A2_SCOPE_ID, G0_EVIDENCE_TRIGGER_ID, G0_TEST_LAYER_ID, G0_CORRECTION_ID]) {
    ...
    if ([POSTMERGE_RANGE_CORRECTION_ID, CLOSEOUT_POLICY_CORRECTION_ID, CI_TRIGGER_SPLIT_ID].includes(packageId)) {
      const diff = readGitDiff({ base: fixedBase, head, worktree: false });
      const result = analyzePackageGate({ ...diff, adrTextByPath: { [definition.adrPath]: readAdrText(definition.adrPath, head, false) }, base: fixedBase, head, worktree: false });
      if (result.packageId !== packageId) throw new Error(`${packageId} correction candidate structurally resolves as ${result.packageId}`);
    } else validateGatePrepBase(head);
    return { base: fixedBase, head, packageId };
  }
  return undefined;
}
function readAuditedText(path, head) {
  let bytes;
  try { bytes = gitBuffer(['show', `${head}:${path}`]); }
  catch { throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} cannot read ${path}`); }
  try { return UTF8_DECODER.decode(bytes); }
  catch { throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} rejects invalid UTF-8 in ${path}`); }
}
function auditedAddedText(base, head) {
  const diff = gitText(['diff', '--no-ext-diff', '--no-color', '--unified=0', base, head, '--', ...A2_CLOSEOUT_EVIDENCE_FILES]);
  const additions = [];
  let inHunk = false;
...
function validateA2CloseoutAdditions(base, head) {
  const additions = auditedAddedText(base, head);
  const normalized = stripRenderedHtmlComments(additions, A2_CLOSEOUT_EVIDENCE_ID)
    .normalize('NFKC')
    .replace(/\p{Default_Ignorable_Code_Point}/gu, '')
    .replace(/[`*~>#\[\](){}|]/g, ' ');
  if (/\bg0_evidence_[^\s:=]+\s*[:=]/i.test(normalized)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} additions cannot mint or rewrite G0 evidence fields`);
  if ([...normalized.matchAll(/\b(\d+)\s*\/\s*42\b/g)].some((match) => match[1] !== '9')) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} additions contradict the frozen 9/42 total`);
  if (/RMD-TASK-00[23][^。；;！？!?\n]{0,80}(?:=|:|为|保持)?\s*(?:closed|completed|已关闭|关闭|已完成|完成)(?=$|[\s,，。；;！？!?])/i.test(normalized)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} additions cannot close RMD-TASK-002 or RMD-TASK-003`);
  const packageSubject = '(?:A[2-5]|B2a[2-5]|RP-02B2a[2-5])';
  const positiveAuthorization = [
    `${packageSubject}[^。；;！？!?\\n]{0,48}(?:已(?:经)?(?:获得|获)?授权|授权(?:已经)?生效|获准(?:启动|开始|进入|开发|实施|推进)|approved\\s+for\\s+(?:implementation|development)|is\\s+(?:authorized|allowed|unlocked|released))`,
    `(?<!不)(?:允许|准予|批准)[^。；;！？!?\\n]{0,36}${packageSubject}(?:[^。；;！？!?\\n]{0,24}(?:启动|开始|进入|开发|实施|推进))?`,
    `\\b(?:authorized|allowed|unlocked|released)\\b[ \\t,，:：]{1,8}\\b${packageSubject}\\b`,
  ];
  if (positiveAuthorization.some((source) => new RegExp(source, 'i').test(normalized))) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} additions cannot authorize A2-A5 or downstream implementation`);
}
function validateA2CloseoutEvidence(base, head) {
  validateA2CloseoutAdditions(base, head);
  const documents = Object.fromEntries([...A2_CLOSEOUT_EVIDENCE_FILES].map((path) => [path, readAuditedText(path, head)]));
  const verification = documents[A2_CLOSEOUT_EVIDENCE_VERIFICATION];
  const status = documents['docs/reviews/main-control-status.md'];
  const eventLedger = documents['docs/reviews/main-control-event-ledger.md'];
  const issueLedger = documents['docs/remediation/issue-ledger.md'];
  const acceptanceMatrix = documents['docs/remediation/acceptance-matrix.md'];
  const requireMatch = (path, text, pattern, label) => {
    if (!pattern.test(text)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} ${path} missing or conflicts with ${label}`);
  };
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /\| admitted_candidate \| `e8e37cd892570e3abee961374ec3512a373f6400` \|/, 'admitted candidate receipt');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /\| admitted_candidate_tree \| `749fe41f42285dc9115351ddcbc675e77d428cb7` \|/, 'candidate tree receipt');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /\| admitted_manifest_digest \| `11e82805b419a97f598c5b4a9595eb897584c92add2e9d0071184ff9e414c1ba` \|/, 'manifest digest receipt');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /\| squash_delivery_head \| `dc193dbbd3ac1970f571fd618f12902a4033994c` \|/, 'squash delivery receipt');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /\| post_merge_gate_head \| `9f04986469a3e409b3ce887390e8830cbdfe9493` \|/, 'post-merge policy predecessor');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /trusted_replay_run: 29977969717 completed\/success/, 'trusted replay receipt');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /^RMD-TASK-002: partial$/m, 'RMD-TASK-002 partial boundary');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /^RMD-TASK-003: open$/m, 'RMD-TASK-003 open boundary');
  requireMatch(A2_CLOSEOUT_EVIDENCE_VERIFICATION, verification, /^issue_closed_count: 9\/42 unchanged$/m, '9/42 total boundary');
  for (const boundary of ['真实 MySQL', '真实付费 provider', '真实媒体', 'E6']) if (!verification.includes(boundary)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} verification must preserve the unproven boundary: ${boundary}`);
  if (!/ARCH\/SECURITY `APPROVED P0=0\/P1=0\/P2=1`/.test(verification) || !/TEST\/QUALITY `APPROVED P0=0\/P1=0\/P2=0`/.test(verification)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} verification must preserve both independent review verdicts`);
  requireMatch('docs/reviews/main-control-event-ledger.md', eventLedger, /^accepted_candidate: e8e37cd892570e3abee961374ec3512a373f6400$/m, 'accepted candidate event');
  requireMatch('docs/reviews/main-control-event-ledger.md', eventLedger, /^accepted_candidate_tree: 749fe41f42285dc9115351ddcbc675e77d428cb7$/m, 'candidate tree event');
  requireMatch('docs/reviews/main-control-event-ledger.md', eventLedger, /^squash_delivery_head: dc193dbbd3ac1970f571fd618f12902a4033994c$/m, 'delivery event');
  requireMatch('docs/reviews/main-control-event-ledger.md', eventLedger, /^post_merge_gate_head: 9f04986469a3e409b3ce887390e8830cbdfe9493$/m, 'post-merge event');
  requireMatch('docs/reviews/main-control-event-ledger.md', eventLedger, /trusted replay `29977969717` completed\/success/, 'trusted replay event');
  requireMatch('docs/reviews/main-control-event-ledger.md', eventLedger, /RMD-TASK-002 保持 partial、RMD-TASK-003 保持 open、总账保持 9\/42/, 'open issue event boundary');
  requireMatch('docs/reviews/main-control-status.md', status, /RP-02B2a2 trusted replay `29977969717` 全绿/, 'trusted replay status');
  requireMatch('docs/reviews/main-control-status.md', status, /总账 9\/42、RMD-TASK-002=partial、RMD-TASK-003=open/, 'status total and issue boundary');
  requireMatch('docs/remediation/issue-ledger.md', issueLedger, /^\| RMD-TASK-002 \|[^\n]*\| partial \|/m, 'RMD-TASK-002 ledger row');
  requireMatch('docs/remediation/issue-ledger.md', issueLedger, /^\| RMD-TASK-003 \|[^\n]*\| open \|/m, 'RMD-TASK-003 ledger row');
  if (/^\| RMD-TASK-00[23] \|[^\n]*\| closed \|/m.test(issueLedger)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} issue ledger cannot close RMD-TASK-002 or RMD-TASK-003`);
  requireMatch('docs/remediation/acceptance-matrix.md', acceptanceMatrix, /candidate `e8e37cd`[^\n]*trusted replay `29977969717`/, 'A2 acceptance receipt');
  requireMatch('docs/remediation/acceptance-matrix.md', acceptanceMatrix, /`9\/42`/, 'acceptance total boundary');
  requireMatch('docs/remediation/acceptance-matrix.md', acceptanceMatrix, /`RMD-TASK-002=partial`/, 'acceptance RMD-TASK-002 boundary');
  requireMatch('docs/remediation/acceptance-matrix.md', acceptanceMatrix, /`RMD-TASK-003=open`/, 'acceptance RMD-TASK-003 boundary');
}
function policyCloseoutRange(input, head) {
  if (!input.policySource) return undefined;
  assertUsableSha('POLICY_SOURCE', input.policySource); assertGitCommit('POLICY_SOURCE', input.policySource);
  if (!isAncestor(input.policySource, head)) return undefined;
  const files = changedFiles(input.policySource, head);
  if (!files.includes(A2_CLOSEOUT_EVIDENCE_VERIFICATION)) return undefined;
  const commitAndParents = gitText(['rev-list', '--parents', '-n', '1', head]).trim().split(/\s+/);
  if (commitAndParents.length !== 2 || commitAndParents[1] !== input.policySource) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} must be one direct child commit of the repository-controlled policy source`);
  const diff = readGitDiff({ base: input.policySource, head, worktree: false });
  const result = analyzePackageGate({ ...diff, adrTextByPath: {}, base: input.policySource, head, worktree: false });
  if (result.packageId !== A2_CLOSEOUT_EVIDENCE_ID) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} structurally resolves as ${result.packageId}`);
  validateA2CloseoutEvidence(input.policySource, head);
  return { base: input.policySource, head, packageId: A2_CLOSEOUT_EVIDENCE_ID, admissionKind: 'closeout' };
}
export function resolvePackageGateRange(input) {
  ...
  if (event === 'pull_request_target') {
    ...
    const closeout = policyCloseoutRange(input, head);
    if (closeout) {
      const mergeBase = gitText(['merge-base', eventBase, head]).trim();
      if (mergeBase !== input.policySource || eventBase !== input.policySource) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} pull request base and merge-base must equal the repository-controlled policy source`);
      return { ...closeout, eventBase };
    }
  ...
  if (event === 'pull_request') {
    ...
    if (correction) {
      if (base !== correction.base) throw new Error(`${correction.packageId} pull request must have fixed merge-base ${correction.base}`);
      return correction;
    }
  ...
  if (event === 'push') {
    ...
    const correction = correctionBootstrapRange(head), closeout = policyCloseoutRange(input, head);
    if (base === ZERO_SHA) {
      ...
      if (closeout) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} cannot be admitted from a newly created ref`);
    ...
    if (closeout) {
      if (input.pushRef !== 'refs/heads/main') throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} push delivery requires refs/heads/main`);
      if (base !== input.policySource || !isAncestor(base, head)) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} push before must equal the repository-controlled policy source`);
      return closeout;
    }
    if (correction) {
      if (base === correction.base) return correction;
      if (correctionBootstrapRange(base)?.packageId === correction.packageId) return correction;
  ...
  if (event === 'workflow_dispatch') {
    ...
    const correction = correctionBootstrapRange(head);
    if (correction) {
      if (base !== correction.base) throw new Error(`${correction.packageId} manual replay requires fixed base ${correction.base}`);
      return correction;
    }
    if (!isAncestor(base, head)) throw new Error('RP-02B2a package gate rejects manual base that is not an ancestor of head');
    if (input.authorizedCandidateSha) throw new Error('RP-02B2a admitted candidate replay requires trusted default-branch repository_dispatch');
    return externallyAuthorizedRange(input, head) ?? { base: fixedPackageBase(base, head), head };
  }
  if (event === 'repository_dispatch') {
    const base = input.manualBase, head = input.manualHead;
    if (input.eventRef !== 'refs/heads/main') throw new Error('RP-02B2a repository replay requires refs/heads/main workflow context');
    assertUsableSha('MANUAL_BASE', base); assertUsableSha('MANUAL_HEAD', head); assertGitCommit('MANUAL_BASE', base); assertGitCommit('MANUAL_HEAD', head);
    if (!isAncestor(base, head)) throw new Error('RP-02B2a package gate rejects repository replay base that is not an ancestor of head');
    if (!input.authorizedCandidateSha) throw new Error('RP-02B2a repository replay requires a complete admitted candidate receipt');
    return trustedSquashDeliveryRange(input, base, head, { requireMainRef: input.eventRef });
  }
...
function validateGatePrepBaseUncached(base) {
  ...
  if (commitAndParents.length === 2 && commitAndParents[1] === G0_A2_SCOPE_BASELINE_SHA) {
    ...
    if (acceptedC3.packageId !== G0_EVIDENCE_TRIGGER_ID) throw new Error(`${G0_A2_SCOPE_ID} baseline must retain accepted ${G0_EVIDENCE_TRIGGER_ID}`);
    validateAcceptedGatePrepEvidence({ gateSource: ACCEPTED_G0_C3_SHA, evidenceHead: G0_A2_SCOPE_BASELINE_SHA, expectedGatePackage: G0_EVIDENCE_TRIGGER_ID });
    const { files, netAdditions } = readGitDiff({ base: G0_A2_SCOPE_BASELINE_SHA, head: base, worktree: false });
    const adrTextByPath = { [G0_A2_SCOPE_ADR]: readAdrText(G0_A2_SCOPE_ADR, base, false) };
    const result = analyzePackageGate({ files, netAdditions, adrTextByPath, base: G0_A2_SCOPE_BASELINE_SHA, head: base, worktree: false });
    if (result.packageId !== G0_A2_SCOPE_ID) throw new Error(`RP-02B2a2 baseline is not a verified ${G0_A2_SCOPE_ID} package`);
    verifyA2ScopeCorrectionContracts({ head: base, worktree: false });
    return result;
  }
  ...
  if (commitAndParents.length === 2 && commitAndParents[1] === G0_TEST_LAYER_BASELINE_SHA) {
    ...
    const { files, netAdditions } = readGitDiff({ base: G0_TEST_LAYER_BASELINE_SHA, head: base, worktree: false });
    const adrTextByPath = { [G0_TEST_LAYER_ADR]: readAdrText(G0_TEST_LAYER_ADR, base, false) };
    const result = analyzePackageGate({ files, netAdditions, adrTextByPath, base: G0_TEST_LAYER_BASELINE_SHA, head: base, worktree: false });
    if (result.packageId !== G0_TEST_LAYER_ID) throw new Error(`RP-02B2a2 baseline is not a verified ${G0_TEST_LAYER_ID} package`);
    verifyTrustedAdmissionWorkflowContract({ workflowPath: TRUSTED_ADMISSION_WORKFLOW, head: base });
    verifyTestLayerCorrectionContracts({ head: base, worktree: false });
    return result;
  }
  ...
  verifyCorrectionEvidenceParentWorkflows({ head: base, worktree: false });
  verifyCorrectionAuthorityContracts({ head: base, worktree: false });
  return result;
}
...
function validateAcceptedGatePrepEvidence({ gateSource, evidenceHead, expectedGatePackage = G0_A2_SCOPE_V5_ID }) {
  ...
  const receipt = validateGatePrepEvidencePublication({ base: gateSource, head: evidenceHead, worktree: false, expectedGatePackage });
...
function verifyCorrectionEvidenceParentWorkflows({ head, worktree }) {
  const selectedHead = worktree ? undefined : head;
  const rp01bPath = '.github/workflows/rp01b-dom.yml';
  const rp01b = parseWorkflowYaml(rp01bPath, selectedHead);
  const expectedRp01b = parseWorkflowYaml(rp01bPath, G0_CORRECTION_BASELINE_SHA);
  expectedRp01b.on.push.paths = [
    expectedRp01b.on.push.paths[0],
    '.github/workflows/rp02b2a-admission.yml',
    'scripts/rp02b2a-package-gate.*',
    ...expectedRp01b.on.push.paths.slice(1)
  ];
  if (JSON.stringify(canonicalJson(rp01b)) !== JSON.stringify(canonicalJson(expectedRp01b))) {
    throw new Error(`${G0_CORRECTION_ID} ${rp01bPath} must add only the frozen G0 admission and package-gate push paths`);
  }
  const governancePath = '.github/workflows/remediation-governance.yml';
  const governance = parseWorkflowYaml(governancePath, selectedHead);
  const expectedGovernance = parseWorkflowYaml(governancePath, G0_CORRECTION_BASELINE_SHA);
  expectedGovernance.on.push.branches = ['main', 'codex/**'];
  if (JSON.stringify(canonicalJson(governance)) !== JSON.stringify(canonicalJson(expectedGovernance))) {
    throw new Error(`${G0_CORRECTION_ID} ${governancePath} must add only the default main push branch`);
  }
}
function verifyCorrectionAuthorityContracts({ head, worktree }) {
  const gatePrep = readAdrText(B2A2_GATE_PREP_ADR, head, worktree);
  const evidenceSection = gatePrep.split('## 一次性 E1 证据发布合同')[1] ?? '';
  if (!evidenceSection.includes('`3 files / additions <= 64 / deletions <= 48 / net additions <= 64`') ||
      !evidenceSection.includes('E1 `additions=64`、`deletions=48`、`net=64`') ||
      !evidenceSection.includes('`head_branch=main`')) {
    throw new Error(`${G0_CORRECTION_ID} original E1 authority contract must freeze 3/64/48/64 and head_branch=main`);
  }

  const authority = parseAdr(readAdrText('docs/adr/rp-02b2a2-authority-claim-budget.md', head, worktree));
  ...
  for (const [field, value] of Object.entries(expectedAuthority)) {
    if (authority[field] !== value) throw new Error(`${G0_CORRECTION_ID} A2 authority template ${field} mismatch: ${authority[field] ?? '<missing>'} != ${value}`);
  }

  const packageText = readAdrText('docs/modules/rp-02b2-dispatcher-transport-implementation-package.md', head, worktree);
  const a2Section = packageText.split('### RP-02B2a2 Authoritative Claim And Pre-provider Gate')[1]?.split('\n### ')[0] ?? '';
  if (!packageText.includes('最终修正包冻结为 14 个精确文件') ||
      !a2Section.includes('`20 files / 3,250 net additions`') ||
      !a2Section.includes('`RP-02B2a2-v3 / 20 / 3,250`') ||
      a2Section.includes('`18 files / 1,900 net additions`')) {
    throw new Error(`${G0_CORRECTION_ID} implementation contract must freeze the 14-file correction and A2 v3 20/3250 template`);
  }
}
function verifyTestLayerCorrectionContracts({ head, worktree }) {
  const authorityPath = 'docs/adr/rp-02b2a2-authority-claim-budget.md';
  const acceptedAuthority = readAdrText(authorityPath, ACCEPTED_G0_C1_SHA, false);
  const currentAuthority = readAdrText(authorityPath, head, worktree);
  if (currentAuthority !== acceptedAuthority) throw new Error(`${G0_TEST_LAYER_ID} must preserve the accepted G0-C1 A2 authority template`);
  const oracle = readAdrText(GATE_PREP_TEST_PATH, head, worktree);
  if (!oracle.includes(`ACCEPTED_G0_C1_SHA = "${ACCEPTED_G0_C1_SHA}"`) ||
...
export function verifyWorkflowContract({ workflowPath, packageId, testCommand, triggerFile, head }) {
  ...
  const triggers = requireMapping(workflow.on, 'workflow on'), dispatch = requireMapping(triggers.repository_dispatch, 'repository_dispatch');
  assertExactKeys(triggers, ['repository_dispatch', 'push', 'pull_request'], 'workflow triggers');
  assertExactKeys(dispatch, ['types'], 'repository_dispatch');
  const dispatchTypes = requireSequence(dispatch.types, 'repository_dispatch types');
  if (JSON.stringify(dispatchTypes) !== JSON.stringify(['rp02b2a_replay'])) throw new Error('RP-02B2a workflow repository_dispatch type must be exactly rp02b2a_replay');
  const requiredFiles = [...new Set([...Object.values(PACKAGE_DEFINITIONS).flatMap((item) => [...item.manifest]), ...A2_CLOSEOUT_EVIDENCE_FILES])];
  let pushPaths;
  for (const event of ['push', 'pull_request']) {
    const eventConfig = requireMapping(triggers[event], `workflow ${event}`);
    assertExactKeys(eventConfig, event === 'push' ? ['branches', 'paths'] : ['paths'], `workflow ${event}`);
    if (event === 'push') {
      const branches = requireSequence(eventConfig.branches, 'workflow push branches');
      if (JSON.stringify(branches) !== JSON.stringify(['main'])) throw new Error('RP-02B2a workflow push branches must be exactly main');
    }
    ...
    if (event === 'push') pushPaths = paths;
    else if (JSON.stringify(paths) !== JSON.stringify(pushPaths)) throw new Error('RP-02B2a workflow pull request paths must match main push paths');
  }
  ...
  const expectedCheckoutRef = "${{ github.event_name == 'repository_dispatch' && github.sha || github.event_name == 'pull_request' && github.event.pull_request.head.sha || github.event.after }}";
  if (checkoutWith.ref !== expectedCheckoutRef) throw new Error('RP-02B2a workflow checkout ref is not bound to the pull request head, push head, or current trusted manual-replay gate');
  ...
  const rangeStep = findNamedStep(steps, 'Resolve B2a package gate range'), resolverStep = findNamedStep(steps, 'Resolve B2a package command'), selfCheckStep = findNamedStep(steps, 'Verify B2a workflow contract'), evidenceStep = findNamedStep(steps, 'Verify G0 evidence parent runs'), budgetStep = findNamedStep(steps, 'Check package diff budget');
  ...
  const expectedStepIdentities = [`uses:${checkoutAction}`, `uses:${setupNodeAction}`, 'name:Resolve B2a package gate range', 'name:Resolve B2a package command', 'name:Verify B2a workflow contract', 'name:Verify G0 evidence parent runs', 'run:npm ci', 'run:./node_modules/.bin/playwright install chromium', 'run:npm run test:rp01c', 'run:npm run test:rp02a', 'run:selected-package-command', 'run:npm test -w @ai-shortvideo/api', 'run:npm run test:e2e:rp01a', 'run:npm run test:governance', 'run:npm run typecheck', 'run:npm run build -w @ai-shortvideo/api', 'run:npm run prisma:validate -w @ai-shortvideo/api', 'run:git diff --check', 'name:Check package diff budget'];
  ...
  const expectedStepKeys = [['uses', 'with'], ['uses', 'with'], ['name', 'id', 'env', 'run'], ['name', 'id', 'env', 'run'], ['name', 'env', 'run'], ['name', 'if', 'env', 'run'], ['run'], ['run'], ['run'], ['run'], ['env', 'run'], ['run'], ['run'], ['run'], ['run'], ['run'], ['run'], ['run'], ['name', 'env', 'run']];
  steps.forEach((step, index) => assertExactKeys(step, expectedStepKeys[index], `rp01c-fixtures step ${index + 1}`));
  assertStepEnv(rangeStep, { EVENT_NAME: '${{ github.event_name }}', EVENT_REF: '${{ github.ref }}', WORKFLOW_SHA: "${{ github.event_name == 'pull_request' && github.event.pull_request.head.sha || github.sha }}", PUSH_REF: '${{ github.ref }}', PUSH_CREATED: '${{ github.event.created }}', PUSH_BEFORE_SHA: '${{ github.event.before }}', PUSH_AFTER_SHA: '${{ github.event.after }}', PR_BASE_SHA: '${{ github.event.pull_request.base.sha }}', PR_HEAD_SHA: '${{ github.event.pull_request.head.sha }}', MANUAL_BASE_SHA: '${{ github.event.client_payload.base_sha }}', MANUAL_HEAD_SHA: '${{ github.event.client_payload.head_sha }}', B2A_POLICY_SOURCE_SHA: '${{ vars.RP02B2A_POLICY_SOURCE_SHA }}', B2A_GATE_SOURCE_SHA: '${{ vars.RP02B2A_GATE_SOURCE_SHA }}', B2A_G0_EVIDENCE_SHA: '${{ vars.RP02B2A_G0_EVIDENCE_SHA }}', B2A_AUTHORIZED_PACKAGE_ID: '${{ vars.RP02B2A_AUTHORIZED_PACKAGE_ID }}', B2A_AUTHORIZED_PREDECESSOR_SHA: '${{ vars.RP02B2A_AUTHORIZED_PREDECESSOR_SHA }}', B2A_AUTHORIZED_CANDIDATE_SHA: '${{ vars.RP02B2A_AUTHORIZED_CANDIDATE_SHA }}', B2A_AUTHORIZED_CANDIDATE_REF: '${{ vars.RP02B2A_AUTHORIZED_CANDIDATE_REF }}', B2A_AUTHORIZED_CANDIDATE_TREE: '${{ vars.RP02B2A_AUTHORIZED_CANDIDATE_TREE }}', B2A_AUTHORIZED_MANIFEST_DIGEST: '${{ vars.RP02B2A_AUTHORIZED_MANIFEST_DIGEST }}' }, 'range resolver');
  assertStepEnv(resolverStep, { B2A_BASE_SHA: '${{ steps.b2a-range.outputs.base }}', B2A_HEAD_SHA: '${{ steps.b2a-range.outputs.head }}', B2A_DELIVERY_BASE_SHA: '${{ steps.b2a-range.outputs.delivery_base }}', B2A_DELIVERY_HEAD_SHA: '${{ steps.b2a-range.outputs.delivery_head }}', B2A_POLICY_SOURCE_SHA: '${{ vars.RP02B2A_POLICY_SOURCE_SHA }}', B2A_GATE_SOURCE_SHA: '${{ vars.RP02B2A_GATE_SOURCE_SHA }}', B2A_G0_EVIDENCE_SHA: '${{ vars.RP02B2A_G0_EVIDENCE_SHA }}', B2A_AUTHORIZED_PACKAGE_ID: '${{ vars.RP02B2A_AUTHORIZED_PACKAGE_ID }}', B2A_AUTHORIZED_PREDECESSOR_SHA: '${{ vars.RP02B2A_AUTHORIZED_PREDECESSOR_SHA }}', B2A_AUTHORIZED_CANDIDATE_SHA: '${{ vars.RP02B2A_AUTHORIZED_CANDIDATE_SHA }}', B2A_AUTHORIZED_CANDIDATE_TREE: '${{ vars.RP02B2A_AUTHORIZED_CANDIDATE_TREE }}', B2A_AUTHORIZED_MANIFEST_DIGEST: '${{ vars.RP02B2A_AUTHORIZED_MANIFEST_DIGEST }}' }, 'package resolver');
  ...
  assertStepEnv(budgetStep, { GOVERNANCE_BASE_SHA: '${{ steps.b2a-range.outputs.delivery_base }}', GOVERNANCE_HEAD_SHA: '${{ steps.b2a-range.outputs.delivery_head }}' }, 'package diff budget');
  ...
  assertShellBlock(budgetStep, budgetCommands(), 'package diff budget');
  ...
  if (packageId || testCommand) { const definition = PACKAGE_DEFINITIONS[packageId], fallback = packageId === FALLBACK_PACKAGE_ID && testCommand === FALLBACK_TEST_COMMAND, evidence = packageId === GATE_PREP_EVIDENCE_ID && testCommand === GATE_PREP_EVIDENCE_COMMAND, closeout = packageId === A2_CLOSEOUT_EVIDENCE_ID && testCommand === A2_CLOSEOUT_EVIDENCE_COMMAND; if ((!definition || definition.testCommand !== testCommand) && !fallback && !evidence && !closeout) throw new Error(`RP-02B2a workflow package command mismatch: ${packageId}/${testCommand}`); }
...
export function verifyTrustedAdmissionWorkflowContract({ workflowPath = TRUSTED_ADMISSION_WORKFLOW, head } = {}) {
  ...
  const acceptedDigests = head && TRUSTED_ADMISSION_WORKFLOW_LEGACY_HEADS.has(head)
    ? new Set([TRUSTED_ADMISSION_WORKFLOW_LEGACY_SHA256, TRUSTED_ADMISSION_WORKFLOW_CANONICAL_SHA256])
    : new Set([TRUSTED_ADMISSION_WORKFLOW_CANONICAL_SHA256]);
  if (!acceptedDigests.has(digest)) throw new Error(`RP-02B2a trusted admission workflow canonical contract mismatch: ${digest}`);
...
function rangeCommands() {
  const authorization = '--policy-source "$B2A_POLICY_SOURCE_SHA" --gate-source "$B2A_GATE_SOURCE_SHA" --g0-evidence-sha "$B2A_G0_EVIDENCE_SHA" --authorized-package-id "$B2A_AUTHORIZED_PACKAGE_ID" --authorized-predecessor-sha "$B2A_AUTHORIZED_PREDECESSOR_SHA" --authorized-candidate-sha "$B2A_AUTHORIZED_CANDIDATE_SHA" --authorized-candidate-tree "$B2A_AUTHORIZED_CANDIDATE_TREE" --authorized-manifest-digest "$B2A_AUTHORIZED_MANIFEST_DIGEST"';
  return ['set -euo pipefail', 'if [[ "$EVENT_NAME" == "repository_dispatch" ]]; then', '[[ "$B2A_AUTHORIZED_CANDIDATE_REF" =~ ^refs/tags/rp02b2a-admitted-[a-f0-9]{12,40}$ ]] || { echo "authorized candidate ref must use the durable rp02b2a admission tag namespace" >&2; exit 1; }', 'git fetch --no-tags --force origin "$B2A_AUTHORIZED_CANDIDATE_REF:refs/remotes/origin/rp02b2a-admitted-candidate"', 'test "$(git rev-parse refs/remotes/origin/rp02b2a-admitted-candidate^{commit})" = "$B2A_AUTHORIZED_CANDIDATE_SHA"', 'fi', 'if [[ "$EVENT_NAME" == "push" ]]; then', GATE_ENV_COMMAND + ` node scripts/rp02b2a-package-gate.mjs --print-range --event push --push-ref "$PUSH_REF" --push-created "$PUSH_CREATED" --push-before "$PUSH_BEFORE_SHA" --push-head "$PUSH_AFTER_SHA" ${authorization} >> "$GITHUB_OUTPUT"`, 'elif [[ "$EVENT_NAME" == "pull_request" ]]; then', 'PR_MERGE_BASE="$(git merge-base "$PR_BASE_SHA" "$PR_HEAD_SHA")"', '{', 'echo "base=$PR_MERGE_BASE"', 'echo "head=$PR_HEAD_SHA"', 'echo "delivery_base=$PR_MERGE_BASE"', 'echo "delivery_head=$PR_HEAD_SHA"', '} >> "$GITHUB_OUTPUT"', 'elif [[ "$EVENT_NAME" == "repository_dispatch" ]]; then', GATE_ENV_COMMAND + ` node scripts/rp02b2a-package-gate.mjs --print-range --event repository_dispatch --event-ref "$EVENT_REF" --manual-base "$MANUAL_BASE_SHA" --manual-head "$MANUAL_HEAD_SHA" ${authorization} >> "$GITHUB_OUTPUT"`, 'else', 'echo "RP-01C only accepts pull requests, trusted main pushes, or default-branch repository_dispatch replay ranges" >&2', 'exit 1', 'fi', 'BASE="$(grep \'^base=\' "$GITHUB_OUTPUT" | tail -n1 | cut -d= -f2-)"', 'HEAD="$(grep \'^head=\' "$GITHUB_OUTPUT" | tail -n1 | cut -d= -f2-)"', 'DELIVERY_BASE="$(grep \'^delivery_base=\' "$GITHUB_OUTPUT" | tail -n1 | cut -d= -f2-)"', 'DELIVERY_HEAD="$(grep \'^delivery_head=\' "$GITHUB_OUTPUT" | tail -n1 | cut -d= -f2-)"', 'if [[ -z "$BASE" || -z "$HEAD" || -z "$DELIVERY_BASE" || -z "$DELIVERY_HEAD" || "$BASE" =~ ^0+$ || "$HEAD" =~ ^0+$ || "$DELIVERY_BASE" =~ ^0+$ || "$DELIVERY_HEAD" =~ ^0+$ ]]; then', 'echo "B2a package gate requires explicit non-zero analysis and delivery ranges" >&2', 'exit 1', 'fi', 'test "$(git rev-parse HEAD)" = "$(git rev-parse "$WORKFLOW_SHA")"'];
}
function budgetCommands() { return ['set -euo pipefail', 'ADR_ARGS=()', "while IFS= read -r -d '' path; do", 'case "$path" in', 'docs/adr/*.md) ADR_ARGS+=(--adr "$path") ;;', 'esac', 'done < <(git diff --name-only -z "$GOVERNANCE_BASE_SHA" "$GOVERNANCE_HEAD_SHA")', 'npm run governance:git-budget -- --base "$GOVERNANCE_BASE_SHA" --head "$GOVERNANCE_HEAD_SHA" "${ADR_ARGS[@]}"']; }
function evidenceRunCommands() { return ['set -euo pipefail', 'test "$B2A_BASE_SHA" = "$EVIDENCE_PARENT"', `IFS=',' read -r -a RUNS <<< "$EVIDENCE_RUNS"`, 'test "${#RUNS[@]}" -eq 4', 'declare -A SEEN_LABELS=() SEEN_RUN_IDS=()', 'for item in "${RUNS[@]}"; do', 'label="${item%%:*}"', 'run_id="${item#*:}"', 'case "$label" in', 'rp01a) expected="RP-01A backend E2E"; workflow_file="rp01a-e2e.yml"; expected_path=".github/workflows/$workflow_file" ;;', 'rp01b) expected="RP-01B admin DOM tests"; workflow_file="rp01b-dom.yml"; expected_path=".github/workflows/$workflow_file" ;;', 'rp01c) expected="RP-01C deterministic fixtures"; workflow_file="rp01c-fixtures.yml"; expected_path=".github/workflows/$workflow_file" ;;', 'governance) expected="Remediation governance"; workflow_file="remediation-governance.yml"; expected_path=".github/workflows/$workflow_file" ;;', '*) exit 1 ;;', 'esac', '[[ "$run_id" =~ ^[1-9][0-9]+$ ]] || { echo "parent run id is invalid" >&2; exit 1; }', 'test -z "${SEEN_LABELS[$label]+x}" || { echo "duplicate workflow label" >&2; exit 1; }', 'test -z "${SEEN_RUN_IDS[$run_id]+x}" || { echo "duplicate workflow run id" >&2; exit 1; }', 'SEEN_LABELS[$label]=1', 'SEEN_RUN_IDS[$run_id]=1', `IFS=$'\\t' read -r expected_workflow_id repository_path < <(gh api "repos/$GITHUB_REPOSITORY/actions/workflows/$workflow_file" --jq '[.id,.path] | @tsv')`, '[[ "$expected_workflow_id" =~ ^[1-9][0-9]+$ ]] || { echo "workflow id lookup is invalid" >&2; exit 1; }', 'test "$repository_path" = "$expected_path" || { echo "workflow repository path mismatch" >&2; exit 1; }', `IFS=$'\\t' read -r name workflow_path workflow_id event run_head head_branch status conclusion < <(gh api "repos/$GITHUB_REPOSITORY/actions/runs/$run_id" --jq '[.name,.path,.workflow_id,.event,.head_sha,.head_branch,.status,.conclusion] | @tsv')`, 'test "$name" = "$expected" || { echo "workflow name mismatch" >&2; exit 1; }', 'case "$workflow_path" in', '"$expected_path") ;;', '"$expected_path"@*) workflow_ref="${workflow_path#"$expected_path"@}"; [[ "$workflow_ref" =~ ^[A-Za-z0-9][A-Za-z0-9._/@+-]{0,254}$ ]] || exit 1 ;;', '*) echo "workflow path mismatch" >&2; exit 1 ;;', 'esac', 'test "$workflow_id" = "$expected_workflow_id" || { echo "workflow id mismatch" >&2; exit 1; }', 'test "$event" = "push" || { echo "workflow event mismatch" >&2; exit 1; }', 'test "$run_head" = "$EVIDENCE_PARENT" || { echo "workflow head mismatch" >&2; exit 1; }', 'test "$head_branch" = "main" || { echo "workflow branch mismatch" >&2; exit 1; }', 'test "$status" = "completed" || { echo "workflow status mismatch" >&2; exit 1; }', 'test "$conclusion" = "success" || { echo "workflow conclusion mismatch" >&2; exit 1; }', 'git cat-file -e "${EVIDENCE_PARENT}:${expected_path}"', 'git cat-file -e "${B2A_HEAD_SHA}:${expected_path}"', 'git diff --quiet "$EVIDENCE_PARENT" "$B2A_HEAD_SHA" -- "$expected_path"', 'done']; }
...
export function parseWorkflowYaml(workflowPath, head) {
  ...
  try {
    ...
    const detail = error?.stderr?.toString().trim() || error?.message || String(error); throw new Error(`RP-02B2a trusted workflow YAML structure parse failed: ${detail}`);
  }
  try {
...
function runCliUncached(argv = process.argv.slice(2)) {
  ...
  if (args.get('print-range') === 'true') {
    const range = resolvePackageGateRange({ event: args.get('event'), eventRef: args.get('event-ref'), eventBase: args.get('event-base'), eventHead: args.get('event-head'), policySource: args.get('policy-source'), gateSource: args.get('gate-source'), g0EvidenceSha: args.get('g0-evidence-sha'), prBaseRef: args.get('pr-base-ref'), prHead: args.get('pr-head'), pushRef: args.get('push-ref') ?? args.get('push-base-ref'), pushCreated: args.get('push-created'), pushBefore: args.get('push-before'), pushHead: args.get('push-head'), manualBase: args.get('manual-base'), manualHead: args.get('manual-head'), authorizedPackageId: args.get('authorized-package-id'), authorizedPredecessorSha: args.get('authorized-predecessor-sha'), authorizedCandidateSha: args.get('authorized-candidate-sha'), authorizedCandidateTree: args.get('authorized-candidate-tree'), authorizedManifestDigest: args.get('authorized-manifest-digest') });
    console.log(`base=${range.base}`); console.log(`head=${range.head}`); console.log(`delivery_base=${range.deliveryBase ?? range.base}`); console.log(`delivery_head=${range.deliveryHead ?? range.head}`); console.log(`range_mode=${range.rangeMode ?? 'direct'}`); console.log(`admission_kind=${range.admissionKind ?? 'business'}`); return;
  }
  ...
  let authoritativeEventBase, admissionKind = 'business';
  if (event === 'pull_request_target') {
    ...
    const range = resolvePackageGateRange({ event, eventBase: args.get('event-base'), eventHead: args.get('event-head'), policySource: args.get('policy-source'), gateSource: args.get('gate-source'), authorizedPackageId: args.get('authorized-package-id'), authorizedPredecessorSha: args.get('authorized-predecessor-sha') });
    base = range.base; head = range.head; authoritativeEventBase = range.eventBase; admissionKind = range.admissionKind ?? 'business';
  ...
  const deliveryBase = args.get('delivery-base') ?? process.env.B2A_DELIVERY_BASE_SHA, deliveryHead = args.get('delivery-head') ?? process.env.B2A_DELIVERY_HEAD_SHA;
  if ((deliveryBase && !deliveryHead) || (!deliveryBase && deliveryHead)) throw new Error('RP-02B2a package gate requires delivery base/head together');
  let deliveryReceipt;
  if (deliveryBase && (deliveryBase !== base || deliveryHead !== head)) {
    deliveryReceipt = trustedSquashDeliveryRange({ gateSource: args.get('gate-source'), g0EvidenceSha: args.get('g0-evidence-sha'), authorizedPackageId: args.get('authorized-package-id'), authorizedPredecessorSha: args.get('authorized-predecessor-sha'), authorizedCandidateSha: args.get('authorized-candidate-sha') ?? process.env.B2A_AUTHORIZED_CANDIDATE_SHA, authorizedCandidateTree: args.get('authorized-candidate-tree') ?? process.env.B2A_AUTHORIZED_CANDIDATE_TREE, authorizedManifestDigest: args.get('authorized-manifest-digest') ?? process.env.B2A_AUTHORIZED_MANIFEST_DIGEST }, deliveryBase, deliveryHead);
    if (!deliveryReceipt || deliveryReceipt.base !== base || deliveryReceipt.head !== head) throw new Error('RP-02B2a delivery receipt does not bind the analyzed authorization range');
  }
  ...
  if (result.packageId === A2_CLOSEOUT_EVIDENCE_ID) {
    ...
    if (!policySource) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} requires an explicit repository-controlled policy source`);
    const closeout = policyCloseoutRange({ policySource }, effectiveHead);
    if (!closeout || closeout.base !== base) throw new Error(`${A2_CLOSEOUT_EVIDENCE_ID} base must equal the repository-controlled policy source`);
  ...
  if (result.packageId === G0_CORRECTION_ID) {
    if (worktree) {
      const parents = gitText(['rev-list', '--parents', '-n', '1', effectiveHead]).trim().split(/\s+/);
      if (parents.length !== 2 || parents[1] !== G0_CORRECTION_BASELINE_SHA) throw new Error(`${G0_CORRECTION_ID} worktree must remain the fixed baseline direct child`);
      verifyTrustedAdmissionWorkflowContract({ workflowPath: TRUSTED_ADMISSION_WORKFLOW });
      verifyWorkflowContract({ workflowPath: '.github/workflows/rp01c-fixtures.yml' });
      verifyCorrectionEvidenceParentWorkflows({ head: effectiveHead, worktree: true });
      verifyCorrectionAuthorityContracts({ head: effectiveHead, worktree: true });
    } else validateGatePrepBase(effectiveHead);
  }
  if (result.packageId === G0_TEST_LAYER_ID) {
    if (worktree) {
      const parents = gitText(['rev-list', '--parents', '-n', '1', effectiveHead]).trim().split(/\s+/);
  ...
  if (result.packageId === G0_A2_SCOPE_ID) {
    if (worktree) {
      const parents = gitText(['rev-list', '--parents', '-n', '1', effectiveHead]).trim().split(/\s+/);
      if (parents.length !== 2 || parents[1] !== G0_A2_SCOPE_BASELINE_SHA) throw new Error(`${G0_A2_SCOPE_ID} worktree must remain the fixed E1 baseline direct child`);
      validateAcceptedGatePrepEvidence({ gateSource: ACCEPTED_G0_C3_SHA, evidenceHead: G0_A2_SCOPE_BASELINE_SHA, expectedGatePackage: G0_EVIDENCE_TRIGGER_ID });
  ...
  if (result.packageId === CI_TRIGGER_SPLIT_ID) {
    if (worktree) {
      const parents = gitText(['rev-list', '--parents', '-n', '1', effectiveHead]).trim().split(/\s+/);
      if (parents.length !== 2 || parents[1] !== CI_TRIGGER_SPLIT_BASELINE_SHA) throw new Error(`${CI_TRIGGER_SPLIT_ID} worktree must remain the fixed main baseline direct child`);
    }
    verifyWorkflowContract({ workflowPath: '.github/workflows/rp01c-fixtures.yml', head: worktree ? undefined : effectiveHead });
  }
  ...
  const closeoutManifestDigest = result.packageId === A2_CLOSEOUT_EVIDENCE_ID ? packageManifestDigest(A2_CLOSEOUT_EVIDENCE_DEFINITION, { base, head: effectiveHead }) : undefined;
  ...
  if (githubOutput) {
    console.log(`package_id=${result.packageId}`); console.log(`test_command=${result.testCommand}`); console.log(`files=${result.files}`); console.log(`net_additions=${result.netAdditions}`); console.log(`admission_kind=${admissionKind}`); if (authoritativeEventBase) console.log(`event_base_sha=${authoritativeEventBase}`); if (deliveryReceipt) { console.log(`range_mode=${deliveryReceipt.rangeMode}`); console.log(`delivery_base_sha=${deliveryReceipt.deliveryBase}`); console.log(`delivery_head_sha=${deliveryReceipt.deliveryHead}`); } if (businessAuthorization) { console.log(`gate_source_sha=${businessAuthorization.gateSource}`); console.log(`g0_evidence_sha=${businessAuthorization.g0EvidenceSha}`); console.log(`g0_evidence_digest=${businessAuthorization.g0EvidenceDigest}`); console.log(`authorized_predecessor_sha=${businessAuthorization.authorizedPredecessor}`); console.log(`candidate_sha=${effectiveHead}`); console.log(`manifest_digest=${businessAuthorization.manifestDigest}`); } if (closeoutManifestDigest) { console.log(`candidate_sha=${effectiveHead}`); console.log(`manifest_digest=${closeoutManifestDigest}`); } if (evidence) { console.log(`evidence_parent=${evidence.parent}`); console.log(`evidence_runs=${evidence.runs}`); } return;
  }
