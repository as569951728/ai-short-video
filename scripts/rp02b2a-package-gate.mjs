...
export function resolvePackageGateRange(input) {
  ...
  if (event === 'push') {
    let base = input.pushBefore; const head = input.pushHead, baseRef = input.pushBaseRef, zeroBefore = base === ZERO_SHA, readyAt = (ref, item) => { try { return parseAdr(gitText(['show', `${ref}:${item.adrPath}`])).status === 'ready'; } catch { return false; } }, changed = (from) => splitNulBuffer(gitBuffer(['diff', '--name-only', '-z', from, head])).map(decodeGitPath); if (!baseRef) throw new Error('RP-02B2a package gate requires explicit PUSH_BASE_REF'); assertUsableSha('PUSH_HEAD', head); assertGitCommit(head); if (!zeroBefore) assertUsableSha('PUSH_BEFORE', base); const landedBase = gitText(['merge-base', baseRef, head]).trim(); assertUsableSha('PUSH_BASE', landedBase); assertGitCommit(landedBase); const beforeReachable = !zeroBefore && spawnSync('git', ['merge-base', '--is-ancestor', base, head], { stdio: 'ignore' }).status === 0; if (zeroBefore || !beforeReachable) base = landedBase; const definitions = Object.values(PACKAGE_DEFINITIONS), a1 = definitions[0], cumulative = changed(landedBase), hasBaseline = spawnSync('git', ['cat-file', '-e', `${BASELINE_SHA}^{commit}`], { stdio: 'ignore' }).status === 0, landedA1 = readyAt(landedBase, a1), laterAdr = definitions.slice(1).some((item) => cumulative.includes(item.adrPath)), fixedA1 = (!landedA1 && hasBaseline && changed(BASELINE_SHA).some((file) => a1.manifest.has(file))) || (landedA1 && cumulative.some((file) => a1.manifest.has(file)) && (cumulative.includes(a1.adrPath) || !laterAdr)), pendingRange = definitions.slice(1).some((item) => cumulative.some((file) => item.manifest.has(file))); if (fixedA1) { assertGitCommit(BASELINE_SHA); execFileSync('git', ['merge-base', '--is-ancestor', BASELINE_SHA, head], { stdio: 'ignore' }); base = BASELINE_SHA; } else if (pendingRange) base = landedBase;
    return { base, head };
  }
