...
function inferRevenueAmount(lead: Pick<RevenueLead, 'amount' | 'offer'>) {
  if (Number(lead.amount) > 0) return Number(lead.amount);
  if (lead.offer.includes('99')) return 99;
  if (lead.offer.includes('100')) return 100;
  if (lead.offer.includes('0')) return 0;
  return 29;
}
...
function buildPaymentReadinessChecklist(leads: RevenueLead[]) {
  const paymentLeads = leads.filter((lead) => lead.status === '已报价' || lead.status === '强意向');
...
下一步：先发出至少 1 次明确报价。对方接受报价后，再发送收款前确认单。
...
目标：对已报价和强意向线索，准备好收款动作，避免对方愿意付费时临场卡住。

${paymentLeads.map((lead, index) => `${index + 1}. ${lead.name}
   状态：${lead.status}
   金额：¥${inferRevenueAmount(lead)}
   报价版本：${lead.offer}
   需求：${lead.need || '未记录'}
   平台位置：${lead.sourceUrl || lead.channel || '未记录'}
   下次跟进：${formatLeadTime(lead.followUpAt)}
...
function App() {
  ...
  const pendingPaymentLeads = revenueLeads.filter((lead) => lead.status === '已报价' || lead.status === '强意向');
  const pendingPaymentCount = pendingPaymentLeads.length;
  ...
  const paymentReadinessChecklist = useMemo(() => buildPaymentReadinessChecklist(revenueLeads), [revenueLeads]);
...
${pendingPaymentCount > 0 ? `- 有 ${pendingPaymentCount} 条待收款线索，先复制收款准备清单并确认交付边界。` : '- 暂无待收款线索，先推动报价。'}
        ...
        {screen === 'revenue' && (
          <section className="workspace">
            ...
            <div className="grid three">
              ...
              <article className={`panel metric ${pendingPaymentCount > 0 ? 'warning-panel' : ''}`}><span>待收款</span><strong>{pendingPaymentCount}</strong></article>
            ...
            <article className={`panel ${pendingPaymentCount > 0 ? 'warning-panel' : ''}`}>
              <div className="panel-title">
                <div>
                  <h3>收款准备清单</h3>
                  <p className="muted">{pendingPaymentCount > 0 ? '有已报价或强意向线索，先准备收款动作和交付边界。' : '当前没有待收款线索，先发出明确报价。'}</p>
                </div>
                <div className="lead-actions">
                  <button className="secondary" onClick={() => setRevenueFilter('待处理')}><ClipboardList size={16} />查看线索</button>
                  <button className="secondary" onClick={() => copyText(paymentReadinessChecklist, '收款准备清单', 'payment-readiness-checklist')}><Copy size={16} />复制清单</button>
                </div>
              </div>
              {pendingPaymentLeads.length === 0 ? (
                <div className="empty compact-empty">
                  <CreditCard size={28} />
                  <p>还没有待收款对象。先处理报价机会，至少发出 1 次明确报价。</p>
                </div>
              ) : (
                <div className="action-queue">
                  {pendingPaymentLeads.slice(0, 5).map((lead, index) => (
                    <div className="queue-row" key={lead.id}>
                      <span>{index + 1}</span>
                      <div>
                        <strong>{lead.name}</strong>
                        <small>{lead.status} / ¥{inferRevenueAmount(lead)}</small>
                      </div>
                      <p>先发收款前确认单，准备项目收款码或收款链接。</p>
                      <button className="secondary" onClick={() => editRevenueLead(lead)}>去收款</button>
                    </div>
                  ))}
                </div>
              )}
              <textarea id="payment-readiness-checklist" className="sr-only-copy" readOnly value={paymentReadinessChecklist} />
            </article>
